...
alpha    = OLS intercept of r on rb over [t-250, t-1] (daily; ×252 to annualize for display)
AR_t     = r_t - beta * rb_t
...
Alpha is the drift the market does not explain: on a day the benchmark does
not move, the regression expects `r_t = alpha`. It is a trailing statistical
residual, not a forecast, and it is **not** Jensen's alpha — subtracting the
risk-free rate needs a risk-free series this project does not carry, and is
deliberately left as follow-up work. The chart shows alpha annualized
(alpha × 252) and labeled as a historical residual.

Every flagged day decomposes as `r_t ≈ beta * rb_t + AR_t`: the market-driven
component (beta times the benchmark return) plus the idiosyncratic component
(the abnormal return). The chart's attribution card shows both parts, and the
render step verifies the identity rather than assuming it.
...
`beta`, `alpha`, `sigma_60`, and `z` are all persisted per move so the chart can display the bar each day had to clear and the factor exposure behind it.
