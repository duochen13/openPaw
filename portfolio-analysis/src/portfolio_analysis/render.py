...
def _kpi_data(store: Store, symbol: str) -> dict[str, Any] | None:
    """Business-KPI panels for the per-stock chart (issues #29 + #36).

    EDGAR panels (#29) come first, then hand-entered manual panels (#36),
    each carrying ``"source": "manual"`` so the template can label them.
    The NOW RPO deceleration flag is computed off the stored EDGAR RPO
    series with thresholds from kpi_manual.yaml. None when the ticker has
    ...
    if manual_cfg is not None:
        ...
        floor, drop = manual_cfg.rpo_yoy_floor, manual_cfg.rpo_pp_drop
    else:
        floor = manual_kpis_module.DEFAULT_RPO_YOY_FLOOR
        drop = manual_kpis_module.DEFAULT_RPO_PP_DROP
    if not metrics:
        return None
    out: dict[str, Any] = {"metrics": metrics}
    if "rpo" in series:
        out["rpo_alert"] = manual_kpis_module.rpo_deceleration_flag(
            series["rpo"], yoy_floor=floor, pp_drop=drop
        )
    return out
