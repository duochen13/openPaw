...
def event_dashboard_data(portfolio: Portfolio, store: Store, calendar: MacroSource) -> dict[str, Any]:
    ...
    for kind, rows in calendar.catalog().items():
        for row in rows:
            events.append({"type": kind, "date": row["date"], "url": row["url"], "impact": {s: _impact(prices[s], row["date"]) for s in symbols}})
...
<style>body{{font:14px system-ui;margin:0;background:#f5f4ef;color:#21231f}}main{{max-width:1100px;margin:auto;padding:28px}}h1{{font:40px Georgia;font-weight:500}}.controls{{display:flex;gap:12px;flex-wrap:wrap;margin:20px 0}}select{{padding:10px;border:1px solid #ccc;border-radius:6px;background:#fffefa}}table{{border-collapse:collapse;width:100%;background:#fffefa}}td,th{{padding:11px;text-align:right;border-bottom:1px solid #ddd}}td:first-child,th:first-child{{text-align:left}}.up{{color:#176b3a}}.down{{color:#ae3d35}}a{{color:#955424}}</style><main><p>OPENPAW / EVENT STUDY</p><h1>Price response around macro events</h1><p>Choose a release type, date, and instrument. Returns use the last trading close before the release and the next 1, 5, or 20 sessions. Timing is association, not causation.</p><div class="controls"><label>Event <select id="type"></select></label><label>Date <select id="date"></select></label><label>Instrument <select id="symbol">{options}</select></label></div><p id="source"></p><table><thead><tr><th>Window</th><th>Return</th></tr></thead><tbody id="body"></tbody></table></main><script type="application/json" id="data">{payload}</script><script>const d=JSON.parse(document.getElementById('data').textContent),$=x=>document.getElementById(x),types=[...new Set(d.events.map(e=>e.type))];$('type').innerHTML=types.map(x=>`<option>${{x}}</option>`).join('');function dates(){{const t=$('type').value;$('date').innerHTML=d.events.filter(e=>e.type===t).map(e=>`<option value="${{e.date}}">${{e.date}}</option>`).join('');}}function draw(){{const e=d.events.find(e=>e.type===$('type').value&&e.date===$('date').value),s=$('symbol').value,i=e.impact[s];$('source').innerHTML=`<a href="${{e.url}}" target="_blank">Source calendar</a> · ${{e.type}} release on ${{e.date}}`;const rows=[['Release session vs prior close',i.before_1d],['1 session after',i.after_1d],['5 sessions after',i.after_5d],['20 sessions after',i.after_20d]];$('body').innerHTML=rows.map(([n,v])=>`<tr><td>${{n}}</td><td class="${{v>=0?'up':'down'}}">${{v==null?'n/a':(v*100).toFixed(2)+'%'}}</td></tr>`).join('');}}$('type').onchange=()=>{{dates();draw();}};$('date').onchange=draw;$('symbol').onchange=draw;dates();draw();</script>''')
    return target
