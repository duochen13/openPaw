...
def render_dashboard(portfolio: Portfolio, out_dir: Path) -> Path:
    ...
    if portfolio.indices:
        rows.append('<h2 class="group">Indices</h2>')
        for index in portfolio.indices:
            symbol = html.escape(index.symbol, quote=True)
            name = html.escape(index.name)
            href = f"{symbol}.html"
            options.append(
                f'<option value="{symbol}.html">{symbol} · {name} (index)</option>'
            )
            rows.append(
                f'<a class="card" href="{href}"><strong>{symbol}</strong>'
                f'<span>{name}</span><em>Open chart →</em></a>'
...
<h1>Stocks in context.</h1><p class="muted">Choose a stock to inspect its price history, unusual moves, and collected event evidence. Each chart is self-contained and works offline.<br><a href="factors.html" style="color:var(--accent)">Compare Beta &amp; Alpha across all stocks →</a></p>
