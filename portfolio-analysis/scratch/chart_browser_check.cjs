...
(async()=>{
 ...
 const setDates=async(a,b)=>{await page.locator('#start').fill(a);await page.locator('#end').fill(b);await page.locator('#end').dispatchEvent('change');};
 await setDates('2024-04-01','2024-04-30');
 assert.match(await page.locator('#chart-title').textContent(),/Apr 1, 2024 — Apr 30, 2024/);
 await page.reload();
 assert.equal(await page.locator('#start').inputValue(),'2024-04-01');
 assert.equal(await page.locator('#end').inputValue(),'2024-04-30');
 await setDates('2024-04-28','2024-04-27');
 assert.equal(await page.locator('#range-error').isVisible(),true);
 await setDates('2010-01-01','2024-04-30');
 assert.equal(await page.locator('#range-error').isVisible(),true);
 await setDates('2024-04-27','2024-04-28');
 assert.match(await page.locator('#axis-note').textContent(),/No trading sessions/);
 await setDates('2024-04-25','2024-04-25');
